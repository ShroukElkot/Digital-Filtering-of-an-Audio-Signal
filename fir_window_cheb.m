function Hd = fir_window_cheb(filterlength, Fpass, Fsample, ripple)
    % Parameters
    ...
    % Apply the Chebyshev window
    chebyshev_window = chebwin(N, ripple); % Chebyshev window with specified ripple
    filter_coeffs = sinc_func .* chebyshev_window'; % Windowed sinc (FIR filter)

    % Plot the filter coefficients
    figure;
    stem(n, filter_coeffs, 'filled');
    title('FIR Filter Coefficients (Chebyshev Window)');
    xlabel('Sample Index');
    ylabel('Amplitude');
    grid on;
