function Hd = fir_window_blackman(filterlength, Fpass, Fsample)
    % Parameters
    ...
    filter_coeffs = sinc_func .* blackman_window'; % Windowed sinc (FIR filter)

    % Plot the filter coefficients
    figure;
    stem(n, filter_coeffs, 'filled');
    title('FIR Filter Coefficients (Blackman Window)');
    xlabel('Sample Index');
    ylabel('Amplitude');
    grid on;
